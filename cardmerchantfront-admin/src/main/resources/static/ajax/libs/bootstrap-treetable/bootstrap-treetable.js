...
(function ($) {
    ...
    $.fn.bootstrapTreeTable = function (options, param) {
        ...
        options.mutipleCode = options.mutipleCode;
                ...
                $.each(options.columns, function (i, column) {
                    if (column.field != 'selectItem') {
                        ...
                        if (typeof column.visible == "undefined" || column.visible == true) {
                            let s = '<li role="menuitem"><label><input type="checkbox" checked="checked" data-field="' + column.field + '" value="' + column.field + '" > ' + column.title + '</label></li>';
                            _li = $(s);
                        } else {
                            let s1 = '<li role="menuitem"><label><input type="checkbox" data-field="' + column.field + '" value="' + column.field + '" > ' + column.title + '</label></li>';
                            _li = $(s1);
        ...
        var formatData = function (data) {
            ...
            $.each(data, function (index, item) {
                ...
                item.isShow = false;
                // 顶级节点校验判断
                let itemid = item[options.code];


                let parentid = item[options.parentCode];
                var _defaultRootFlag = $.inArray(itemid, parentCodes) > 0 && !rootFlag;

                if (!parentid || (_root ? (parentid == options.rootIdValue) : _defaultRootFlag)) {
                    rootFlag = true;
                    if (!target.data_list["_root_"]) {
                        target.data_list["_root_"] = [];
                    }
                    if (!target.data_obj["id_" + itemid]) {
                        target.data_list["_root_"].push(item);
                    }
                } else {
                    if (!target.data_list["_n_" + parentid]) {
                        target.data_list["_n_" + parentid] = [];
                    }

                    // if (!target.data_obj["id_" + itemid]) {
                        target.data_list["_n_" + parentid].push(item);
                    // }

                }


                target.data_obj["id_" + itemid] = item;
            });
        ...
        var recursionNode = function (parentNode, lv, row_id, p_id) {
            var $tbody = target.find("tbody");
            let n_nodes = "_n_" + parentNode[options.code];
            var _ls = target.data_list[n_nodes];
