    ...
    $.extend({
        ...
        table: {
            ...
            onLoadSuccess: function (data) {
                ...
                //$('[data-i18name]').each(render);
                // $('#bootstrap-table').dragtable && $('#bootstrap-table').dragtable();
            },
            // 表格销毁
        ...
        treeTable: {
            ...
            init: function (options) {
                ...
                $.bttTable = $('#' + options.id).bootstrapTreeTable({
                    ...
                    columns: options.columns,                           // 显示列信息（*）
                    mutipleCode: options.mutipleCode,
        ...
        form: {
            ...
            reset: function (formId, tableId) {
                table.set(tableId);
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                let $Element = $("#" + currentId)[0];
                $Element.reset();
                var optionsfirst = true;
                $($Element).find('select').selectpicker && $($Element).find('select').selectpicker('refresh');
            ...
            resetDate: function (formId, tableId) {
                table.set(tableId);
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                let $Element = $("#" + currentId)[0];
                $Element.reset();
                ...
                if (startTime && endTime) {
                    startTime.val(getDefaultToday(0));
                    endTime.val(getDefaultToday(0, true));
                }
                if (table.options.type == table_type.bootstrapTable) {
                    if ($.common.isEmpty(tableId)) {
                        $("#" + table.options.id).bootstrapTable('refresh');
        ...
        operate: {
            ...
            operateurl: function (name) {
                ...
                $.modal.confirm("确认要" + name + "选中的" + rows.length + "条数据吗?", function () {
                    var url = table.options.operateurl;
                    var data = {"ids": rows.join()};
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            edit: function (id, title) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open(title ? title : "修改" + table.options.modalName, url);
                } else {
                    $.modal.open(title ? title : "修改" + table.options.modalName, $.operate.editUrl(id));
                }
            },

            // 修改信息
            editpop: function (id, URL) {
                table.set();
                ...
                $.modal.open(table.options.modalName, url ? url : $.operate.popeditUrl(id));

            },    // 修改信息

            // 修改信息
            editpoptwo: function (modalName, URL) {
                table.set();
                $.modal.open(modalName, URL);

            },    // 修改信息

            proviewUrls: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.proviewUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
            ...
            proview: function (id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.proviewUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("查看" + table.options.modalName, url,undefined,undefined,function () {
                        layer.closeAll()
                    },['关闭']);
                } else {
                    $.modal.open("查看" + table.options.modalName, $.operate.proviewUrls(id),undefined,undefined,function () {
                        layer.closeAll()
                    },['关闭']);
                }
            },
            ...
            // 修改访问地址
            popeditUrl: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.popupdateUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
