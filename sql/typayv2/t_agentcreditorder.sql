...
CREATE TABLE t_agentcreditorder
(
    orderindex                BIGINT(29) AUTO_INCREMENT NOT NULL COMMENT '索引'
        PRIMARY KEY,
...
CREATE INDEX createtime_index
    ON t_agentcreditorder (createtime);

CREATE INDEX name_index
    ON t_agentcreditorder (name);

CREATE INDEX name_remark_index
    ON t_agentcreditorder (remark, name);

CREATE INDEX orderid_idx
    ON t_agentcreditorder (orderid);
